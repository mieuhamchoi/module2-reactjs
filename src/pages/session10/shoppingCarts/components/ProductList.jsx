    ...
    </div>
    {
      ...
      <div key={randomId()} className='productCards'>
        ...
        <div className='productCards_des'>
            ...
            <span className='des_productPrice'>{convertToVND(product.price)} Stock: {product.stock}</span>
            <span className='des_detail'>
              {product.des}
            </span>
        </div>
        <form onSubmit={(e) => formSubmit(e, product.id)} className='productCards_buytools'>
            <input defaultValue={1} className='buytools_amounts' name="quantity" type="number" min="1" max={product.stock}/>
            <button disabled={product.stock ? false : true} type="submit" className="btn btn-danger buytools_amounts-buyBtn">Mua Ngay</button>
        </form>
      </div>
      )
    }
